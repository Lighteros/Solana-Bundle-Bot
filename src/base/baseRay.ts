...
export class BaseRay {
  ...
  constructor(input: BaseRayInput) {
    ...
    // let iter = this.pools.entries()
    // for (let i = 0; i < 2; ++i) {
    //   let [id, info]: [string, LiquidityPoolJsonInfo] = iter.next().value
    //   log({info})
    //   // if (i > 20) {
    //   //   log({ id, bd: info.baseDecimals, qd: info.quoteDecimals})
    //   //   info.programId
    //   // }
    // }
  }
  ...
  async fetchPools() {
    const liquidityJsonStr = await (
      await fetch("https://api.raydium.io/v2/sdk/token/raydium.mainnet.json")
    ).text();
  ...
  > {
    ...
    if (baseLotSize.eq(ZERO)) throw "lot size is too small";
    if (quoteLotSize.eq(ZERO)) throw "tick size or lot size is too small";
