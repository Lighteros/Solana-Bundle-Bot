...
import { TokenMetadataAuthorizationDetails } from "@metaplex-foundation/js/dist/types";
import {
...
export class BaseMpl {
  ...
  async createToken(
    ...
    opts: { decimal?: number; mintAmount?: number; mintKeypair?: web3.Keypair },
  ) {
    const ixs = [];
  ...
  async transfer(input: MPLTransferInput) {
    let { mint, receiver, sender, amount, tokenStandard, isPNFT } = input;
      ...
      const ixs = this.metaplex
        ...
        .transfer({
          ...
          amount: {
            ...
            currency: { decimals: 0, namespace: "spl-token", symbol: "" },
          }, //TODO:
          // amount: null as any,
          fromOwner: sender,
  ...
  async burn(input: BurnInput) {
    ...
    const ixs = this.metaplex
      ...
      .delete({
        ...
        amount: {
          basisPoints: new BN(amount) as any,
          currency: { decimals: decimal, namespace: "spl-token", symbol: "" },
        }, //TODO:
        // amount: token(amount)
      })
      .getInstructions();
  ...
  async getAndCheckTokenName(mint: web3.PublicKey, defaultName = " ") {
    try {
      const metadataAccount = BaseMpl.getMetadataAccount(mint);
      const [mintAccountInfo, metadataAccountInfo] = await this.connection
        .getMultipleAccountsInfo([mint, metadataAccount])
  ...
  static getTokenNameFromAccountInfo(
